...
# ============ 训练加速优化 ============
VAL = True                 # 开启验证。YOLOv8 默认每个 epoch 验证一次，关闭 PLOTS 后验证其实会快很多
PLOTS = False              # 绝对要保持设为 False！(关闭画图可解决90%以上的验证卡顿)
WORKERS = 8                # DataLoader 线程数（可根据 CPU 核心数增加到 8 或 16）
CACHE = False              # 是否将数据集缓存到内存 ('ram') 或硬盘 ('disk') 以加速数据读取，内存够用可设为 True
